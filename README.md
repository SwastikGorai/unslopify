...
The default route is the documented TypeSafe-compatible Gateway endpoint with model `typesafe-ai/jev`. The direct option uses TypeSafe's documented endpoint and `jev-latest`. The current implementation sends `state` as a JSON string because the public TypeSafe quick start documents string state; no silent route or model fallback is used.
...
- `npm run build` copies the auditable MV3 source into `dist/`.
